...
const SelectReviewer  = ({ title, selectedItemValue, setSelectedItemValue, Items, keyName, setItemsVisible, itemsVisible }) => {
  ...
  const renderItem = ({ item }) => (
    <View style={[MyStyles.container]}>
        ...
        style={[]}
        onPress={() => {
          setSelectedItemValue(item.user); //Lưu id của item
  ...
  const selectedItem = Items.find(item => item.user === selectedItemValue);
